...
export default function RealTimeFilter({ searchParams, availableTags }: Props) {
  ...
  const applyFiltersWithNewData = useCallback(
  ...
  return (
    <div className="mb-6 rounded-lg border border-gray-200 bg-white p-4 shadow-xs transition-shadow duration-200 hover:shadow-md dark:border-gray-700 dark:bg-gray-800 dark:hover:shadow-lg dark:hover:shadow-gray-900/20">
      ...
      <div className={`flex items-center gap-3 ${isExpanded && 'mb-4'}`}>
        ...
        <div className="relative min-w-0 flex-1">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-300 bg-white py-2 pl-10 pr-4 text-gray-900 placeholder-gray-500 focus:border-transparent focus:outline-hidden focus:ring-2 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 dark:placeholder-gray-400"
          />
        </div>
        ...
        <div className="space-y-4 border-t border-gray-200 pt-4 dark:border-gray-700">
              ...
              {(['Easy', 'Medium', 'Hard'] as const).map(difficulty => (
                <Button
                  ...
                  className={cn(
                    ...
                    localFilters.difficulty === difficulty &&
                      (difficulty === 'Easy'
                        ? 'border-green-500 bg-green-500 text-white shadow-md shadow-green-500/25 hover:bg-green-600 [&]:text-white'
                        : difficulty === 'Medium'
                          ? 'border-yellow-500 bg-yellow-500 text-white shadow-md shadow-yellow-500/25 hover:bg-yellow-600 [&]:text-white'
                          : 'border-red-500 bg-red-500 text-white shadow-md shadow-red-500/25 hover:bg-red-600 [&]:text-white'),
                  )}
                >
                  <span
                    className={cn(
            ...
            <div>
              ...
              <div className="flex max-h-32 flex-wrap gap-1.5 overflow-y-auto">
                {availableTags.slice(0, 20).map(tag => (
                  <Button
                    key={tag}
                    variant={localFilters.tags === tag ? 'default' : 'outline'}
                    size="sm"
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(
                ['TypeScript', 'JavaScript', 'Python', 'Java', 'C++'] as const
              ).map(language => (
                <Button
                  key={language}
                  variant={
