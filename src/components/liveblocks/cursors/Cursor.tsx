...
// Cursor.module.css migrated to TailwindCSS
...
function CursorComponent({ connectionId }: Props) {
  ...
  return (
    <motion.div
      aria-hidden="true"
      className="absolute top-0 left-0 flex select-none pointer-events-none z-[2]"
      initial={coords}
      animate={coords}
      transition={{
        type: 'spring',
    ...
    >
      <svg viewBox="0 0 23 22" fill="none" xmlns="http://www.w3.org/2000/svg" className="h-4 w-auto shadow-md">
        <path
      ...
      <span
        className="shadow-md -ml-0.5 mt-3.5 rounded px-2 text-sm font-medium h-7 inline-flex items-center"
        style={{
