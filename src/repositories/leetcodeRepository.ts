...
// Supabase 타입을 LeetCode 타입으로 변환하는 함수
...
export const leetcodeRepository = {
  /**
   * LeetCode 문제 목록을 가져옵니다 (Server Side)
   */
  ...
  /**
   * LeetCode 문제 목록을 가져옵니다 (Static Generation)
   */
  async getLeetCodeProblemsStatic(
    filters: LeetCodeFilters = {},
  ): Promise<LeetCodeListResponse> {
    ...
    if (error) {
      console.error('LeetCode 문제 목록 조회 오류 (Static):', error);
      throw new Error('문제 목록을 불러오는데 실패했습니다.');
    }
   ...
   */
  async getLeetCodeProblem(slug: string): Promise<LeetCodeProblemRecord | null> {
    const supabase = await supabaseServer();
   ...
   */
  async getLeetCodeProblemStatic(slug: string): Promise<LeetCodeProblemRecord | null> {
    const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    ...
    if (error) {
      ...
      console.error('LeetCode 문제 조회 오류 (Static):', error);
      throw new Error('문제를 불러오는데 실패했습니다.');
    }
  ...
  /**
   * 사용 가능한 모든 태그를 가져옵니다 (Static)
   */
  async getAvailableTagsStatic(): Promise<string[]> {
    const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    ...
    if (error) {
      console.error('태그 조회 오류 (Static):', error);
      return [];
    }
  ...
  async getAvailableLanguages(): Promise<string[]> {
    ...
    const languages = data?.map(item => item.solution_language).filter(Boolean) || [];
    return Array.from(new Set(languages)).sort();
  },

  /**
   * 사용 가능한 모든 프로그래밍 언어를 가져옵니다 (Static)
   */
  async getAvailableLanguagesStatic(): Promise<string[]> {
    const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
    ...
    if (error) {
      console.error('언어 조회 오류 (Static):', error);
      return [];
    }
  ...
  async getRelatedProblemsStatic(
    ...
    if (error) {
      console.error('관련 문제 조회 오류 (Static):', error);
      return [];
    }
  ...
  /**
   * 모든 문제의 slug를 가져옵니다 (Static Generation용)
   */
  async getAllSlugs(): Promise<string[]> {
    ...
    if (error) {
      console.error('Slug 조회 오류:', error);
      return [];
    }
